...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 위도 : 37.46469200000025, 경도 : 127.0237048 집
        // 위도 : 37.497951699999575, 경도 : 127.02343460000012 강남역
        // 위도 : 37.50446350000012, 경도 : 127.0161415999997 신논현역역

        // 위도 : 37.497944, 경도 : 127.027654 강남역
        // 위도 : 37.504457, 경도 : 127.024515 신논현역역


        mPlaces.add(new MyPlace("신논현역",37.50446350000012, 127.0161415999997, Color.RED));
        mPlaces.add(new MyPlace("집",37.46469200000025,127.0237048, Color.BLUE));
...
//                    MyPlace place = mPlaces.get(0);
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // 메인에서 implement 를 받는게 편하다. 다른 액티비티에서 사용시 context를 계속 받아와야한다
        fusedLocClient = LocationServices.getFusedLocationProviderClient(this);

        // 위치정보 설정 요청 객체
        LocationRequest locRequest = LocationRequest.create();
        locRequest.setPriority(LocationRequest.PRIORITY_HIGH_ACCURACY); // 고성능으로 처리해달라
        locRequest.setInterval(1000); // 1초 마다 한번씩 요청
        ...
        // 요청객체로 갱신정보 설정 (처음처리할것, 콜백, 루프는 자동으로)
...
//            firstChk = false;
            ...
            camPos = new CameraPosition.Builder()
                    .target(latlng)
                    .tilt(45f) // 지도 기울기
                    .bearing(bearing) // 지도 돌림 (자이로스코프 값)
                    .zoom(15f) // 15배 배율
                    .build();
